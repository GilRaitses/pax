...
- ✅ **Manual executions:** All succeed (`gcloud run jobs execute` works perfectly)
- ❌ **Scheduler-triggered executions:** None succeed (no executions created)
- ✅ **Scheduler is triggering:** Cloud Scheduler fires every 30 minutes as configured
- ❌ **Executions not created:** Scheduler HTTP requests do not create Cloud Run job executions

### Investigation Timeline

**November 10, 2025 - Initial Investigation:**
- Created diagnostic scripts to compare manual vs scheduler invocations
- Tested manual execution: ✅ 100% success rate (6/6 collections succeeded)
...
1. ✅ **Test Script Created:** `scripts/2025-11-10/test_single_camera_collection.py`
   - Verifies Cloud Run execution works independently
   - Result: 100% success rate (6/6 manual executions)

2. ✅ **Comparison Script Created:** `scripts/2025-11-10/compare_invocations.py`
   - Analyzes differences between manual and scheduler invocations
   - Checks logs, IAM, OIDC tokens, HTTP headers

3. ✅ **Scheduler Trigger Test:** Manually triggered scheduler to capture exact error
...
**Current Status:**
- Manual collections: ✅ Working
- Scheduled collections: ❌ Failing
...
**Status:** 🔴 **CRITICAL - BLOCKING DATA COLLECTION**
...
- Result: ✅ 100% success (6/6 collections succeeded)
- **Conclusion:** Cloud Run execution works perfectly when triggered manually

**Scheduler vs Manual Comparison:**
- Manual executions: ✅ All succeed
- Scheduler executions: ❌ None found (no executions created)
...
- ✅ `docs/logs/2025-11-10.md` - Added critical issue section
- ✅ Diagnostic scripts created and tested
- ✅ Comparison analysis in progress
- ⏳ Final root cause identification pending
...
The scheduler is:
- ✅ Triggering correctly (every 30 minutes)
- ✅ Sending HTTP POST requests to correct endpoint
- ✅ Using correct OIDC service account (`1019191232687-compute@developer.gserviceaccount.com`)
- ✅ OIDC audience matches URI
- ❌ **OIDC token authentication failing (401 Unauthorized)**
...
2. **Standard practice:** OIDC is the documented standard for Cloud Scheduler → Cloud Run
...
2. **Incremental Debugging Trap:**
   - Started with "fix permissions" → seemed logical
   - Then "fix configuration" → seemed logical
   - Then "wait for propagation" → seemed logical
...
7. **Pattern Matching:**
   - "Cloud Scheduler → Cloud Run" → "use OIDC" (from Services examples)
...
- **Works with:** Cloud Run **Services** ✅
- **Does NOT work with:** Cloud Run **Jobs** ❌
...
- **Works with:** Cloud Run **Jobs** ✅
- **Also works with:** Cloud Run Services ✅
...
- **Services:** Have HTTP endpoints → OIDC validates "who is calling"
- **Jobs:** Invoked via API → OAuth authorizes "what can be done"
...
**Result:**
- ✅ HTTP 200 (Success) in scheduler logs
- ✅ Executions being created successfully
- ✅ No more UNAUTHENTICATED errors
- ✅ Scheduler now working reliably
...
**Status:** ✅ **SOLVED - OAuth tokens working, scheduler operational**
...
**Result:** ✅ Scheduler now successfully creates Cloud Run Job executions. HTTP 200 success, no more authentication errors.
...
1. ✅ **Update deployment script to use OAuth by default** - DONE
...
2. 🔄 **Monitor next scheduled run (2:00 PM ET)** - IN PROGRESS
   - Created `scripts/2025-11-10/monitor_next_run.sh` to monitor executions
   - Will verify execution completes successfully and collects images
   - Current time: 1:55 PM ET, next run at 2:00 PM ET

3. ⏳ **Confirm collection is working consistently**
...
4. ⏳ **Verify execution completes successfully and collects images**
   - Check execution status after 2:00 PM run
   - Verify images appear in GCS bucket
   - Confirm all 82 cameras are being collected

**Status:** 🟢 **RESOLVED - Scheduler operational with OAuth tokens, monitoring next run**
...
- ✅ **Purpose-built for scheduled/batch tasks**
- ✅ **Simpler:** Just run a script/container on a schedule
- ✅ **No HTTP endpoint needed:** Direct execution
- ✅ **Perfect fit:** Exactly what Jobs are designed for
- ✅ **Less overhead:** No request handling, no HTTP server
- ✅ **Better resource usage:** Runs only when scheduled, no idle time

**Cloud Run Services (What We Didn't Use):**
- ❌ **Purpose-built for HTTP endpoints:** Web apps, APIs
- ❌ **More complex:** Need to handle HTTP requests
- ❌ **Requires HTTP endpoint:** Need to expose a URL
- ❌ **Overkill for batch tasks:** Designed for request-response patterns
- ❌ **More overhead:** Service stays available, even when not running
- ❌ **Wrong architecture:** Would need to create an HTTP endpoint just to trigger a batch job

### Architectural Comparison

**With Jobs (Current - Correct):**
```
Scheduler → OAuth Token → Jobs API → Create Execution → Run Script
```
- Direct execution
...
```
Scheduler → OIDC Token → Service HTTP Endpoint → Service Code → 
  → Trigger Job via API → Create Execution → Run Script
```
- Extra HTTP layer
...
4. **Two-Step Process:** Scheduler → Service → Job (instead of Scheduler → Job)
