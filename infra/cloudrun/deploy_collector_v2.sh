...
Optional flags:
  ...
  --manifest        Path to camera manifest (default: data/corridor_cameras_numbered.json)

Example:
  ./deploy_collector_v2.sh --project pax-nyc --region us-central1 --bucket pax-nyc-images
EOF
}
...
SERVICE_ACCOUNT=""
SCHEDULE="*/30 * * * *"
MANIFEST="data/corridor_cameras_numbered.json"
...
IMAGE="gcr.io/$PROJECT/pax-collector-v2"
JOB_NAME="pax-collector-v2"
SCHEDULER_NAME="pax-collector-v2-schedule"
OLD_JOB_NAME="pax-collector"
OLD_SCHEDULER_NAME="pax-collector-schedule"
...
cat > infra/cloudrun/Dockerfile.v2 <<'DOCKERFILE'
...
COPY data/corridor_cameras_numbered.json /app/data/corridor_cameras_numbered.json

# Copy entrypoint
COPY infra/cloudrun/entrypoint_v2.sh /app/entrypoint.sh
...
- name: 'gcr.io/cloud-builders/docker'
  args: ['build', '-f', 'infra/cloudrun/Dockerfile.v2', '-t', '$IMAGE', '.']
images:
- '$IMAGE'
EOF
...
gcloud run jobs deploy "$JOB_NAME" \
  ...
  --image "$IMAGE" \
  --tasks 1 \
  --set-env-vars PAX_REMOTE_PROVIDER=gcs,PAX_REMOTE_BUCKET="$BUCKET",PAX_REMOTE_PREFIX="$PREFIX",PAX_CAMERA_MANIFEST="/app/data/corridor_cameras_numbered.json" \
...
echo ""
echo "=========================================="
echo "Step 5: Create Cloud Scheduler (direct HTTP trigger)"
echo "=========================================="
